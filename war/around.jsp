...
<%@ page import="gozolabs.com.dao.REGISTRATIONDao" %>
<%@ page import="gozolabs.com.SecureStorage" %>
<%@ page import="gozolabs.com.generateHash" %>
<%@ page import="gozolabs.com.AuthenticationUtil" %>
<%@ page import="gozolabs.com.entities.Registration" %>
<html>
  <head>
...
//    window.scrollTo(0,1)
    ...
    elem = document.getElementById('qrcanv');
    qrc = elem.getContext('2d');
    qrc.canvas.width = 518;
    qrc.canvas.height = 518;
    qrc.fillStyle = '#eee';
    qrc.fillRect(0,0,518,518);

}

function doqr() {
    <% generateHash randQR = new generateHash();
    String ranStr = randQR.get(40);
    %>
    d = document;
    ecclevel = 1;
    var randString = new String(d.qrinp.qrinput.value  + "<%=ranStr%>");
    qf = genframe(randString);
    qrc.lineWidth=1;
  ...
  <body onload="setupqr()">
  <%
	    String authKey = AuthenticationUtil.getToken(SecureStorage.USER, SecureStorage.PASSWORD);
  %>
  ...
  <form action="/send" method="post" >
	<table>
		...
		<tr>
			<td>            
			<select id="userNum" name="userNum">
				<option value="--Select--">--Select--</option>
				<%
				List <Registration> regList = REGISTRATIONDao.INSTANCE.listRegistration();
				for (int i = 0; i <regList.size(); i++) {
					String userNo = regList.get(i).getUserNo();
				%>
        ...
        <tr> 
			<td><input type="text" name="msg" id="msg" size="65"/></td>
			
		</tr>
		<tr>
			<td colspan="2" align="left"><input type="submit" name="Submit" value="Send Message"></td>
		</tr>
	</table>
  </form>
     ...
     <table>
        <tr> 
			 <div id="qrdiv" style="background:lightgray; position:absolute; width:360px; height:360px">
  				...
  				<form name="qrinp">
    			<input type="button" value="Go" onclick="doqr()"/><br>
